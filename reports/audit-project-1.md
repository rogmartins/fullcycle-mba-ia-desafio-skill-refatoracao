...
[CRITICAL] Injeção de SQL — Execução Arbitrária de Query
File: app.py:69
...
[CRITICAL] Injeção de SQL — INSERT com Concatenação de Strings
File: models.py:47
...
[CRITICAL] Injeção de SQL — Autenticação via Concatenação de Strings
File: models.py:109
...
[CRITICAL] Injeção de SQL — Construtor Dinâmico de WHERE
File: models.py:285
...
[HIGH] Lógica de Negócio no Controller — Notificações
...
[HIGH] Dado Sensível Exposto na Resposta da API
...
[HIGH] Acoplamento Forte — Acesso Direto ao Banco no Controller
...
[HIGH] Estado Global Mutável — Conexão com Banco de Dados
File: database.py:4
...
[MEDIUM] Validação de Entrada Ausente — Corpo da Requisição de Login
...
[MEDIUM] Queries N+1 — get_pedidos_usuario
File: models.py:186
...
Recommendation: Substituir pelo padrão de carregamento antecipado (eager
...
[MEDIUM] Queries N+1 — get_todos_pedidos
File: models.py:219
...
Recommendation: Aplicar o mesmo padrão de carregamento antecipado descrito para
              `get_pedidos_usuario`. Considerar paginação no endpoint
              para limitar o volume total.

[LOW] Strings Mágicas — Lista de Categorias Hardcoded
...
[LOW] Números Mágicos — Limiares de Desconto
File: models.py:257
