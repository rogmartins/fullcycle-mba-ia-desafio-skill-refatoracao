# REFACTORED: [HIGH] Conexão gerida por requisição (flask.g) em vez de global mutável.
import sqlite3
...
def get_db():
    # REFACTORED: [HIGH] Conexão por contexto de requisição, não mais singleton global.
    if "db" not in g:
        g.db = _connect()
    return g.db
...
def init_db():
    ...
    if cursor.fetchone()[0] == 0:
        ...
        # REFACTORED: [CRITICAL] Senhas armazenadas com hash (antes texto puro em database.py:76).
        usuarios = [
